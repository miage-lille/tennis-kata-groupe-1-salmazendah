  ...
  test('Given players at 0 or 15 points score kind is still POINTS', () => {
   fc.assert(
     fc.property(G.getPoints(), G.getPlayer(), ({ pointsData }, winner) => {
        // Précondition : les deux joueurs ont 0 ou 15 points
      fc.pre(
   ...
   fc.assert(
     fc.property(G.getPoints(), G.getPlayer(), ({ pointsData }, winner) => {
      fc.pre(
        (pointsData.PLAYER_ONE.kind === 'THIRTY' && winner === 'PLAYER_ONE') ||
        (pointsData.PLAYER_TWO.kind === 'THIRTY' && winner === 'PLAYER_TWO')
      );
